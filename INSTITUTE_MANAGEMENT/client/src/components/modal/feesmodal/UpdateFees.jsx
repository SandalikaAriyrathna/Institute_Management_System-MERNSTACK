...
export default function UpdateFees(props) {
  ...
  const [payment_slip, setPaymentSlip] = useState("");
...
const PF = "http://localhost:5000/payment_slips/";
...
const handleUpdate = async (e)=> {
  ...
  if (file) {
    ...
    console.log(id)
   await axios.put(`http://localhost:5000/api/OnlineFeeses/${id}`,payment)
  ...
  const getData = async (e)=> {
    ...
    .then(function (response) {
      if(response.data != null){
        ...
        setPaymentSlip(response.data['payment_slip'])
...
<br />
<Form.Group >
<h5  >Current Payment Slip</h5> 
   
  <img src={PF+payment_slip} className='img-fluid rounded' alt='' style={{width:'300px',height:'auto', float:'left'}}/>      
  

</Form.Group>

<Form.Group  > 
  <h5 className='text-end' style={{marginRight:'100px'}}>New Payment Slip</h5> 
      <MDBCol  className='mb-4'>
      {file && (
        <img className='img-fluid rounded' src={URL.createObjectURL(file)} alt="" style={{width:'300px', float:'right'}} /> 
      )} 
      </MDBCol>       
    </Form.Group> 
</Col>
